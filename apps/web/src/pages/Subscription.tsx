...
export default function Subscription() {
  ...
  const handleSubscribe = (planId: string) => {
    if (planId === 'free') {
      alert('Вы уже используете бесплатный план');
      return;
    }
        ...
        <div className="max-w-[1280px] mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            {plans.map(plan => (
              <div
                key={plan.id}
                className={`bg-white rounded-lg overflow-hidden transition-all ${plan.highlighted
                  ? 'border-2 border-purple-700 shadow-lg scale-105'
                  : 'border border-gray-200 shadow-md'
                  }`}
              >
                {plan.badge && (
                  <div className={`py-2 text-center text-white ${plan.highlighted ? 'bg-purple-700' : 'bg-gray-700'
                    }`}>
                    {plan.badge}
                  </div>
                )}
